...
const lookup = (login, property) => {
  ...
  const ehProperty = Object.keys(users[0]).some((key) => key === property);
  const ehUser = users.some((user) => user.login === login);

  if(!ehProperty) throw new Error('Could not find property');

  if(!ehUser) throw new Error('Could not find user');
